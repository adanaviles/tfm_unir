...
def feature_eng_transactions(train_or_test: str):
    ...
    transactions['paid_per_day'] = np.where(transactions['payment_plan_days'] != 0,
                                            (transactions['actual_amount_paid'] / transactions['payment_plan_days']),
                                            transactions['actual_amount_paid'])

    transactions = transactions.groupby('msno').agg(
...
def feature_eng_members(train_or_test: str):
    ...
    # For registration_init_time we have data up to 2/28/2017 replace outliers
    members['registration_init_time'] = members['registration_init_time'].fillna(np.mean(members[
...
def feature_eng_logs(train_or_test: str):
    ...
    columns_to_clip = ['num_25', 'num_50', 'num_75', 'num_985', 'num_100', 'num_unq', 'total_secs',
                      'days_between_logins']

    for i, chunk in enumerate(
        pd.read_csv(SRC_DIR / "data/internal/user_logs.csv", chunksize=chunk_size, parse_dates=['date']), start=1):
        ...
        chunk_agg_total = chunk.groupby('msno').agg(
        ...
        monthly_agg = chunk.groupby(["msno", "year", "month"]).agg(
        ...
        means_monthly_pivot = monthly_agg.pivot(index='msno',
                                                columns='month_rank',
                                                values=monthly_agg.columns[3:-1]) # not adding year, month and msno
